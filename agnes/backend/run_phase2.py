...
Usage:
    cd agnes/
    python -m backend.run_phase2 [options]

Options:
    --skip-iherb            Skip iHerb product scraping (Tavily)
    --skip-suppliers        Skip supplier enrichment (Tavily + LLM fallback)
    --skip-fda              Skip OpenFDA enforcement risk checks
    --skip-opencorporates   Skip OpenCorporates entity verification
    --skip-compliance       Skip LLM compliance inference
    --skip-contradictions   Skip contradiction detection
    --use-mock              Fill missing data with mocks before enrichment
"""
...
async def run_phase2(
    ...
    if not skip_opencorporates:
        ...
        logger.info("\n--- Step 4: OpenCorporates Entity Verification ---")
        # Reuse already-fetched supplier list if Step 3 also ran
        if skip_fda:
            suppliers = get_all_suppliers()

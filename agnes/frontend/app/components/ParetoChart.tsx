...
export function ParetoChart() {
  ...
  // 5-objective weights matching the backend's NSGA-II + utility formula
  const [alpha, setAlpha] = useState(1.0);   // Savings
  const [beta, setBeta] = useState(1.5);     // Compliance Risk
  const [gamma, setGamma] = useState(1.0);   // Substitution Risk
  const [delta, setDelta] = useState(0.5);   // Reliability Variance
  const [epsilon, setEpsilon] = useState(0.8); // Uncertainty
  ...
  const fetchRerank = useCallback(
    async (a: number, b: number, g: number, d: number, e: number) => {
      setLoading(true);
      try {
  ...
  const schedule = (a: number, b: number, g: number, d: number, e: number) => {
    if (timerRef.current) clearTimeout(timerRef.current);
    timerRef.current = setTimeout(() => fetchRerank(a, b, g, d, e), 150);
  };
  ...
  // Knee point = best all-around balance (highest utility on the frontier)
  const kneePoint =
    frontier.length > 0
      ? frontier.reduce(
          (best, p) => (p.utility_score > best.utility_score ? p : best),
          frontier[0]
        )
      : null;

  // Sort frontier by impact_score so the step-line follows the Pareto shape
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            ...
            size = companies consolidated · opacity = confidence · green dashes = knee point
          </p>
        </div>
        {loading && (
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 10, right: 60, bottom: 28, left: 10 }}>
            ...
            {/* Quadrant fills — relative to X_MID=0.45 and Y_MID=0.5 */}
            ...
            <Scatter
              ...
              shape={<DominatedDot cx={0} cy={0} payload={{} as RerankPoint} />}
            >
              {dominated.map((entry) => (
                <Cell key={`d-${entry.id}`} />
              ))}
            ...
            {/* Pareto-optimal frontier — dots + dashed step-line */}
            <Scatter
              name="Frontier"
              data={sortedFrontier}
              onClick={handleClick}
              style={{ cursor: "pointer" }}
              shape={<FrontierDot cx={0} cy={0} payload={{} as RerankPoint} />}
              line={{
                stroke: "#f59e0b",
                strokeWidth: 1.5,
                strokeDasharray: "4 3",
